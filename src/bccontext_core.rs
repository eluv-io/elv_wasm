...
impl<'a> BitcodeContext {
    ...
    /// fetch_link_async resolves the fabric file link
    /// # Arguments
    /// * `link` : fabric link
    /// # Returns
    /// * UTF8 [u8] slice containing the resolved link
    /// ```rust
    /// fn do_something<'s>(bcc: &'s mut elvwasm::BitcodeContext) -> wapc_guest::CallResult {
    ///   let res = bcc.fetch_link_async(serde_json::from_str("/qfab/hq_somehash/file/assets/foo.jpg")?)?;
    ///   Ok(res)
    /// }
    /// ```
    pub fn fetch_link_async(&'a self, link: serde_json::Value) -> CallResult {
        let fetch_params = json!
        (
          {
            "link": link,
        ...
        self.call_function("FetchLinkAsync", fetch_params, "core")
    }
