...
fn fab_file_to_image<'a>(bcc: &&mut elvwasm::BitcodeContext, stream_id:&str, asset_path:&str) -> image::ImageResult<image::DynamicImage>{
...
fn do_img<'a>(bcc: &'a mut elvwasm::BitcodeContext) -> CallResult{
...
fn do_proxy<'a>(bcc: &'a mut elvwasm::BitcodeContext) -> CallResult{
  let http_p = &bcc.request.params.http;
  let qp = &http_p.query;
